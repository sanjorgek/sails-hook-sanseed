          ...
          function(itemRelation, cbMap) {
            ...
            for(var i=0; i<modelAssociations.length; i++){
              if(modelAssociations[i].alias === itemRelation){
                var associatedName = modelAssociations[i][modelAssociations[i].type];
                return sails.models[associatedName].find(item.oneTo[itemRelation]).limit(1).exec(function(err, associatedModels) {
              ...
              if(err) next2(err);
                ...
                function(itemRelation, cbMap) {
                  ...
                  for(var i=0; i<modelAssociations.length; i++){
                    if(modelAssociations[i].alias === itemRelation){
                      var associatedName = modelAssociations[i][modelAssociations[i].type];
                      return sails.models[associatedName].find({or: item.manyTo[itemRelation]}).exec(function(err, associatedModels) {
                        if(!err || associatedModels!=[]){
