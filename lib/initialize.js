...
const doSeed = (sails, seed, name, done) => {
  let modelSeed = sails.config.seed.databases[seed][name];
  return (!modelSeed) ? done(new Error("missing model"))
  ...
  return function(cb){       
    sails.after(["hook:orm:loaded", "hook:policies:loaded"], function() {
      ...
      const seedModel = (seed, name, done) => {
        ...
        return (checkModel) ? done(new Error("Database missing"))
          : doSeed(sails, seed, name, done);
      };
      sails.seed = {
        seedAll (name, done) {
          const seedAllModels = (seed) => {
            return (!seed) ? done(new Error("Seed not found"))
              : mapLimit(
                Object.keys(seed), 
                1, 
                function(item, next){
