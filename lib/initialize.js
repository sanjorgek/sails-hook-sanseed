    ...
    const mapCreate = (modelSeed, name, done) => {
      if(modelSeed.faker){
        ...
        for(var i = 0; i < limit; i++){
          ...
          schemeFakerJson.push({data: jsonData, oneTo: fakerScheme.oneTo, manyTo: fakerScheme.manyTo});
        }
        modelSeed.scheme = schemeFakerJson;
      }
      mapLimit(modelSeed.scheme, 1, (item, next) => {
        var next2 = next;
        if(modelSeed.migrate==='safe') next2 = (err) => {
          debug(err);
          next();
        };
